...
//u8 msb_tablt[256] = {0x00,0x80,0x40,0xc0,0x20,0xa0,0x60,0xe0,0x10,0x90,0x50,0xd0,0x30,0xb0,0x70,0xf0,
...
u8 GetGray_RGB565(u8 href, u8 row){
    ...
    //while (addr_cnt%2) ;     //buf0 done
    buf0 = *(u8*)address;
    //buf0 = msb_tablt[buf0];
    //while (!(addr_cnt%2)) ;     //buf1 done
    buf1 = *(u8*)(address + 1);
    //buf1 = msb_tablt[buf1];
//    r = (*(u8*)address) & 0xF8;
//    g = (((*(u8*)(address+1)) & 0x07) << 5) + (((*(u8*)(address+1)) & 0xE0) >> 3);
//    b = ((*(u8*)(address+1)) & 0x1F) << 3;

//    r = (buf0) & 0xF8;
//    g = (((buf0) & 0x07) << 5) + (((buf1) & 0xE0) >> 3);
//    b = ((buf1) & 0x1F) << 3;
